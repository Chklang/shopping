...
@Component({
  ...
  templateUrl: './yoursshops.component.html',
  styleUrls: ['./yoursshops.component.css']
})
export class YoursshopsComponent implements OnInit, OnDestroy {
  public columns = null;
  ...
  ngOnInit() {
    ...
    this.logService.getCurrentIdPlayer().then((pIdPlayer: number) => {
      ...
      pShops.forEach((pShop: model.IShop) => {
        if (pShop.owner === null && !this.currentPlayer.isOp) {
          //Can't modify global shops
          return;
        }
        if (pShop.owner !== this.currentPlayer && !this.currentPlayer.isOp) {
          //It's not your shop
          return;
        }
