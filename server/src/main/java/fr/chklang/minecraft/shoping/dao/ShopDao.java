...
public class ShopDao extends AbstractDao<Shop, Long> {
	...
	public Shop get(Long pKey) {
		try {
			...
			PreparedStatement lStatement = lDB.prepare("SELECT name, x_min, x_max, y_min, y_max, z_min, z_max, owner, basemargin, space FROM shopping_shops WHERE id = ?");
			...
			if (!lResultSet.next()) {
				...
				double lBaseMargin = lResultSet.getDouble("basemargin");
				...
				return new Shop(pKey, lName, lX_min, lX_max, lY_min, lY_max, lZ_min, lZ_max, lOwner, lBaseMargin, lSpace);
			}
	...
	public List<Shop> getShopsByOwner(Player pPlayer) {
		try {
			Database lDB = DBManager.getInstance().getDb();
			PreparedStatement lStatement = lDB.prepare("SELECT id, name, x_min, x_max, y_min, y_max, z_min, z_max, basemargin, space FROM shopping_shops WHERE owner = ?");
			if (pPlayer == null) {
			...
			while (lResultSet.next()) {
				...
				double lBaseMargin = lResultSet.getDouble("basemargin");
				long lSpace = lResultSet.getLong("space");
				lResults.add(new Shop(lId, lName, lX_min, lX_max, lY_min, lY_max, lZ_min, lZ_max, pPlayer, lBaseMargin, lSpace));
			}
			return lResults;
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	public List<Shop> getAll() {
		try {
			Database lDB = DBManager.getInstance().getDb();
			PreparedStatement lStatement = lDB.prepare("SELECT id, name, x_min, x_max, y_min, y_max, z_min, z_max, owner, basemargin, space FROM shopping_shops");
			ResultSet lResultSet = lDB.query(lStatement);
			List<Shop> lResults = new ArrayList<>();
			while (lResultSet.next()) {
				long lId = lResultSet.getLong("id");
				...
				lResults.add(new Shop(lId, lName, lX_min, lX_max, lY_min, lY_max, lZ_min, lZ_max, lOwner, lBaseMargin, lSpace));
			}
			return lResults;
		} catch (SQLException e) {
			throw new RuntimeException(e);
